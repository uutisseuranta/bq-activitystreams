...
class TestReadActivity(unittest.TestCase):
    ...
    def test_read_batch_success_and_explosion(self, mock_bq):
        ...
        response = self.client.post("/ap/inbox", json=payload, headers={"Authorization": "Bearer mock-test"})
        self.assertEqual(response.status_code, 201)
        self.assertIn("id", response.json())
    ...
    def test_read_single_string_object(self, mock_bq):
        mock_bq.insert_rows_json.return_value = []
        payload = {"type": "Read", "object": "https://example.com/art1"}
        response = self.client.post("/ap/inbox", json=payload, headers={"Authorization": "Bearer mock-test"})
        self.assertEqual(response.status_code, 201)
    ...
    def test_read_activity_unknown_object_returns_200(self, mock_bq):
        mock_bq.insert_rows_json.return_value = []
        payload = {"type": "Read", "object": "https://example.com/unknown-art"}
    ...
    def test_read_activity_duplicates_succeed(self, mock_bq):
        mock_bq.insert_rows_json.return_value = []
        payload = {"type": "Read", "object": ["https://example.com/art1", "https://example.com/art1"]}
        response = self.client.post("/ap/inbox", json=payload, headers={"Authorization": "Bearer mock-test"})
        self.assertEqual(response.status_code, 201)
    ...
    def test_read_batch_size_exceeded_returns_400(self):
        payload = {"type": "Read", "object": [f"https://example.com/{i}" for i in range(501)]}
        response = self.client.post("/ap/inbox", json=payload, headers={"Authorization": "Bearer mock-test"})
        self.assertEqual(response.status_code, 400)
    ...
    def test_read_long_object_id_returns_422(self):
        ...
        payload = {"type": "Read", "object": long_id}
        response = self.client.post("/ap/inbox", json=payload, headers={"Authorization": "Bearer mock-test"})
        self.assertEqual(response.status_code, 422)
